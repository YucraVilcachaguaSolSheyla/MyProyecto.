 ...
 * @author USER
 */
public class ControladorReportePorMetodoPago {


public class ControladorReportePorEstado {
    
    public ControladorReportePorEstado(ArrayList<RegistroMaestro> registros) {
    }

    public void generarReportePorEstado(String rutaCSV, String rutaSalidaTXT) {
        Map<String, Integer> conteoEstado = new TreeMap<>();
            ...
            if (linea == null) errores.add("Archivo vacio");
                ...
                while ((linea = br.readLine()) != null) {
                    ...
                    try {
                        ...
                        String estado = rm.getLogistica().getEstadoOrden();
                        conteoEstado.put(estado, conteoEstado.getOrDefault(estado, 0) + 1);
        ...
        try (PrintWriter pw = new PrintWriter(new FileWriter(rutaSalidaTXT))) {
            pw.println("REPORTE POR ESTADO");
            pw.println("===================\n");
            for (Map.Entry<String, Integer> e : conteoEstado.entrySet()) {
        ...
        if (!errores.isEmpty()) {
            try (PrintWriter pe = new PrintWriter(new FileWriter(rutaSalidaTXT + ".errores.txt"))) {
                pe.println("ERRORES - Reporte Por Estado");
